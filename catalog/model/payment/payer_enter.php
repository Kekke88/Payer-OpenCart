...
class ModelPaymentPayerenter extends Model {

	public function getMethod($country_id = '', $zone_id = '', $postcode = '') {
		$this->load->language('payment/payer_enter');

		if ($this->config->get('payer_enter_status')) {
			...
			//
			$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "zone_to_geo_zone WHERE geo_zone_id = '" . (int) $this->config->get('payer_enter_geo_zone_id') . "' AND country_id = '" . (int) $country_id . "' AND (zone_id = '" . (int) $zone_id . "' OR zone_id = '0')");
			//

			if (!$this->config->get('payer_enter_geo_zone_id')) {
				$status = TRUE;
		...
		if ($status) {
			$method_data = array(
				...
				'terms' => '',
				'sort_order' => $this->config->get('payer_enter_sort_order')
			);
		}
